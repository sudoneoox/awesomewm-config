...
awful.layout.layouts = {awful.layout.suit.fair, -- awful.layout.suit.tile,
...
myawesomemenu = {{"manual", terminal .. " -e man awesome"}, {"edit config", editor_cmd .. " " .. awesome.conffile},
                 {"restart", awesome.restart}, {"power off", function()
    awesome.util.spawn_with_shell("systemctl poweroff")
end}, {"reboot", function()
    awesome.util.spawn_with_shell("systemctl reboot")
end}}
...
awful.key({modkey}, "space", function()
    awful.util.spawn("rofi -show run -theme ~/.config/polybar/hack/scripts/rofi/launcher.rasi")
end, {
    description = "launch app runner",
    group = "launcher"
}), -- Lock
awful.key({modkey}, "l", function()
    awful.util.spawn_with_shell(HOME_DIR .. "/.config/awesome/scripts/lock.sh")
end, {
...
awful.key({modkey}, "b", function()
    ...
    description = "open blueman manager",
    group = "launcher"
}))
...
function toggle_layout(c)
    if c.floating then
        naughty.notify({
            text = "Window is now in tiling mode"
        })
        c.floating = false
        awful.layout.set(awful.layout.suit.fair, c) -- Set 'fair' layout
    else
        naughty.notify({
            text = "Window is now in floating mode"
        })
        c.floating = true
        c.ontop = true
        c:raise()
...
}, -- Floating clients.
{
    rule_any = {
        ...
        "Wpa_gui", "veromix", "xtightvncviewer", "Rofi"},
...
-- lock on startup
awful.spawn.with_shell(HOME_DIR .. "/.config/awesome/scripts/lock.sh")
...
-- time sync for my laptop that has linux running on an ssd
awful.spawn.with_shell("timedatectl set-timezone US/Central")
