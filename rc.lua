...
-- Helper Functions
function is_laptop()
    -- Method 1: Check for a battery directory
    if awful.util.spawn_with_shell("test -d /sys/class/power_supply/BAT*") == 0 then
        return true
    end
end


-- add batery icon to the taskbar if the device is a laptop
...
awful.spawn.with_shell("xautolock -time " .. IDLE_LOCK_TIME .. " -locker i3lock -detectsleep &")
